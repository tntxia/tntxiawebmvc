...
public class MultipartStream {
    ...
    public void showHeadTail(String msg) {
    	System.out.println(msg + " head and tail," + head + "," + tail);
    }
    
    public void showBuffer() throws UnsupportedEncodingException {
    	String str = new String(buffer, "ISO-8859-1");
    	System.out.println("buffer"+ str);
    }
    
    public void showBuffer(int pos) {
    	System.out.println("buffer char,,," + buffer[pos]);
    }
     ...
     * @param len
     */
    public void readToBuffer(byte[] b, int off, int len) {
    	System.arraycopy(buffer, head, b, off, len);
    	head += len;
    }
    ...
    public void resetTail(int tail) {
    	this.tail = tail;
    }
     ...
     * @throws IOException 
     */
    public int resetBuffer(int pad) throws IOException {
    	int res = input.read(buffer, pad, bufSize - pad);
    	return res;
    }
     ...
     * 通知字节的读取
     */
    public void noteBytesRead(int bytesRead) {
    	if (notifier != null) {
     ...
     * 移动缓存的尾巴
     */
    public void translateTail(int num) {
    	tail += num;
    	this.showBuffer(tail);
    }
